...
function playerAPI2004() {
    ...
    this.defaultModel = {
        'cmi._children': ['student_id', 'student_name', 'lesson_location', 'credit', 'lesson_status', 'entry', 'score', 'total_time', 'lesson_mode', 'exit', 'session_time'],
        'cmi.student_id': '770b7a652cee065cbb9759ff1a0d46a927c8ba98',
        'cmi.student_name': 'Appleseed, John',
        'cmi.lesson_location': '',
        'cmi.credit': 'credit',
        'cmi.lesson_status': 'not attempted',
        'cmi.entry': 'ab-initio',
        'cmi.score_children': ['raw', 'min', 'max'],
        // TODO: limit score.raw to 100 and check its initial value
        'cmi.score.raw': null,
        'cmi.score.max': null,
        'cmi.score.min': null,
    ...
    this.SetValue = function (varName, varValue) {
        console.log('GetValue => ' + varName + ' = ' + varValue);

        if (varName == 'cmi.completion_status' && varValue == 'completed') {
            saveComplete()
        }
    ...
    this.GetLastError = function () {
        console.log('GetLastError =>' + that.lastError);
        return that.lastError;
    };
    this.GetErrorString = function (code) {
        console.log('GetErrorString =>' + that.errorCodes[code]);
        return that.errorCodes[code];
    };
    this.GetDiagnostic = function (code) {
        console.log('GetDiagnostic');
        return that.errorCodes[code];
    };
